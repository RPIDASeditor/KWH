...
# start bit, stop bit, parity bit, data(BIN)
# the start bit, stop bit, and parity bit value represent is there is one. ie. startBit = 1 means there is a start bit. 
# Could just pad with 0s... depending on how to HEX conversion happens
# Removes the beginning and ending buffer from the data(BIN)
# Parses through the data(BIN)
# Converts to hex
...
#buffer = 28
buffer = 0
#start_bit = 1 #1 indicate there is a start bit
#stop_bit = 1 #1 indicates there is a stop bit
#parity_bit = 0 #0 indicates there is no parity bit

def trim(myinput, num): #this function removes the intial buffer
	myinput = myinput[num::]
	myinput = myinput[::-1]
	myinput = myinput[num+parity_bit::]
	myinput = myinput[::-1]
	return myinput
